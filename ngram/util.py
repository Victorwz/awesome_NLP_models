...
class BigramLanguageModel:
    ...
    def compute_bigram_prob_interpolation(self, beta, alpha):
        ...
        cnt = np.zeros((self.n))
        for i, _ in enumerate(self.bi_prob):
            cnt[i] = np.sum(self.bi_prob[i])
        bi_count = np.sum(self.bi_prob)
        for i, _ in enumerate(self.bi_prob):
            for j, __ in enumerate(self.bi_prob[i]):
                self.bi_prob[i][j] = beta * ((self.bi_prob[i][j] + alpha) / (cnt[i] + alpha * self.n)) + (1 - beta) * ((cnt[j] + alpha) / (bi_count + alpha * self.n ))
        # Do not remove the following normalization check.
        for i in range(self.bi_prob.shape[0]):
            assert abs(1 - np.sum(self.bi_prob[i])) < 1e-5
