...
class CRFLoss(nn.Module):
    ...
    def decode(self, scores):  # B x T x L
        B, T, L = scores.size()
        prev = scores[:, 0, :] + self.start.unsqueeze(0).expand([B, L])  # TODO (B x L)
        back = []
        for i in range(1, T):
            prev, indices = (prev.unsqueeze(2).expand([B, L, L]) + scores[:, i, :].unsqueeze(1).expand(
                [B, L, L]) + self.T.t().unsqueeze(0).expand([B, L, L])).max(dim=1)  # TODO (indices: B x L)
            back.append(indices)

        max_scores, indices = (prev + self.end).max(dim=1)  # TODO (indices: B)
        tape = [indices]
        for i in range(T - 1):
            indices = back[-i-1].gather(dim=1, index=tape[-1].unsqueeze(dim=-1).expand([B, L]))[:, 0]  # TODO
            tape.append(indices)
    ...
    def decode_brute(self, scores):
        ...
        for yseq in itertools.product(list(range(L)), repeat=T):
            targets = torch.LongTensor(yseq).expand(B, T) #yseq T targets BXT
    ...
    def compute_normalizers(self, scores):
        B, T, L = scores.size()
        prev = scores[:, 0, :] + self.start.unsqueeze(0).expand([B, -1])  # TODO (B x L)
        for i in range(1, T):
            prev = torch.logsumexp(prev.unsqueeze(2).expand(-1, -1, L) + \
                    scores[:, i, :].unsqueeze(1).expand(-1, L, -1) + \
                    self.T.t().unsqueeze(0).expand(B, -1, -1), dim = 1)  # TODO: implement only using prev (no new definition)
        normalizers = (prev + self.end).logsumexp(dim=1)  # TODO (B)
        return normalizers
