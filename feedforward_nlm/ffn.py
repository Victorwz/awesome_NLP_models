...
class FFLM(nn.Module):
    ...
    def __init__(self, model, vocab, unk_symbol, init, lr, check_interval, seed,
        ...
        #self.FF = FF(self.batch_size * self.nhis * wdim, hdim, self.batch_size * self.V, nlayers)
        self.FF = FF(self.nhis * wdim, hdim, self.V, nlayers)
    ...
    def forward(self, X, Y, mean=True):  # X (B x nhis), Y (B)
        ...
        #print(X.size(), Y.size())
        #logits = self.FF.forward(self.E(X).view(self.batch_size, -1)).view(self.batch_size, self.V)
        logits = self.FF.forward(self.E(X).view(self.batch_size, -1))
        #print(logits.size())
