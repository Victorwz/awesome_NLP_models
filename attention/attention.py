...
class GlobalAttention(nn.Module):
    ...
    def forward(self, queries, memory_bank, memory_lengths):
        ...
        # c: (B X T' X d)  
        # queries: (B X T' X d)
        # self.linear_out: (2d X d)
        attn_h = torch.tanh(self.linear_out(torch.cat((c, queries), 2)))
    ...
    def score(self, Q, K):
        """
               (BxT'xd) (BxTxd)  ------->  (BxT'xT)
               self.linear_in: input (BXT'Xd) output (BXT'Xd)
        """
        output = self.linear_in(Q)
        L = torch.bmm(output, K.transpose(1,2))
